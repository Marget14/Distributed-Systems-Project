...
public class RoutingProxyServiceImpl implements RoutingProxyService {
    ...
    public RouteWithGeometry getRoute(double fromLat, double fromLon, double toLat, double toLon) {
        // Sanitize base URL: Trim whitespace and remove ALL trailing slashes
        String baseUrl = osrmBaseUrl.trim().replaceAll("/+$", "");

        // Coordinates: {lon},{lat};{lon},{lat} - Force US Locale to ensure dots (.) not commas (,)
        String coordinates = String.format(Locale.US, "%f,%f;%f,%f", fromLon, fromLat, toLon, toLat);

        // Build URL manually
        String url = baseUrl + "/route/v1/driving/" + coordinates +
                "?overview=full&alternatives=false&steps=false&geometries=geojson";

        LOGGER.info("Calling OSRM Route: {}", url);

        try {
            // Use URI.create(url) to prevent RestTemplate from re-encoding
            ResponseEntity<OsrmRouteGeoResponse> response = restTemplate.getForEntity(URI.create(url), OsrmRouteGeoResponse.class);
