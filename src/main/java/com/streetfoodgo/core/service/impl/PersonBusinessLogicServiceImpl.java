    ...
    public CreatePersonResult createPerson(final CreatePersonRequest request, final boolean notify) {
        ...
        if (notify) {
            ...
            final String content = String.format(
                    "Welcome to StreetFoodGo, %s! Your account has been created successfully.",
                    firstName
            );
            this.smsNotificationPort.sendSms(mobilePhoneNumber, content);

            // Email Verification
            String verificationLink = "http://localhost:8080/auth/verify?token=" + person.getVerificationToken();
            String emailBody = String.format("""
                <html>
                <body>
                    <h1>Welcome to StreetFoodGo, %s!</h1>
            ...
            try {
                this.emailPort.sendEmail(emailAddress, "Verify your StreetFoodGo account", emailBody);
            } catch (Exception e) {
                LOGGER.error("Failed to send verification email to {}", emailAddress, e);
            }
        }
